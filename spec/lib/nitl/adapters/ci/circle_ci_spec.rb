...
RSpec.describe Nitl::Adapters::Ci::CircleCi do
  let(:pr_number) { 42 }
  let(:repo_owner) { 'acme' }
  ...
  describe '.build_status' do
    it 'returns nil when api_token is missing' do
      expect(Nitl::Interfaces::Print).to receive(:error).with('ci_api_token is not set in .nitl/secrets.yaml')

      result = described_class.build_status(pr_number, repo_owner, repo_name, nil)

      expect(result).to be_nil
      expect(Nitl::Interfaces::Shell).not_to have_received(:run_command).with(/circleci\.com/)
    end
  ...
  describe '.build_failures' do
    it "returns error when api_token is missing" do
      ...
      expect(result).to eq('ci_api_token is not set in .nitl/secrets.yaml')
    end
