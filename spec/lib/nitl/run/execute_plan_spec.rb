...
RSpec.describe Nitl::Run::ExecutePlan do
  describe '.run' do
    let(:issue_number) { '123' }
    let(:branch_name) { "issue-#{issue_number}" }
    let(:plan_file) { File.join('.nitl', 'plans', "plan_gh_issue_no_#{issue_number}.md") }
      ...
      it 'copies secrets.yaml from main repo to worktree' do
        Dir.mktmpdir do |main_repo|
          Dir.mktmpdir do |worktree_path|
            ...
            FileUtils.mkdir_p(File.join(main_repo, '.nitl', 'plans'))
            ...
            FileUtils.mkdir_p(File.join(worktree_path, '.nitl', 'plans'))
            ...
            allow(Nitl::Interfaces::Repo).to receive(:secrets_file).and_return(worktree_secrets_file)
            ...
            # Mock switch_into_worktree for switching back
            allow(Nitl::Interfaces::Shell).to receive(:switch_into_worktree).with('master') do
              Dir.chdir(main_repo)
            end
    ...
    context 'when worktree already exists' do
      it 'does not copy secrets.yaml' do
        Dir.mktmpdir do |tmpdir|
          ...
          # Create .nitl/plans directory and plan file
          FileUtils.mkdir_p(File.join(tmpdir, '.nitl', 'plans'))
          ...
          Dir.chdir(tmpdir) do
            ...
            expect(Nitl::Interfaces::Shell).not_to receive(:run_command)
              .with('git rev-parse --show-toplevel')
