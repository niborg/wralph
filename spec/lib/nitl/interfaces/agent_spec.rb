...
RSpec.describe Nitl::Interfaces::Agent do
  describe '.run' do
    it 'calls claude with the provided instructions' do
      instructions = 'Test instructions'
      expected_command = /claude -p.*--dangerously-skip-permissions/

      allow(Nitl::Interfaces::Shell).to receive(:run_command).and_return(['test output', '', true])

      described_class.run(instructions)

      expect(Nitl::Interfaces::Shell).to have_received(:run_command) do |cmd|
    ...
    it 'returns the stdout output from claude' do
      ...
      allow(Nitl::Interfaces::Shell).to receive(:run_command).and_return([expected_output, '', true])
    ...
    it 'shell-escapes the instructions to prevent command injection' do
      ...
      allow(Nitl::Interfaces::Shell).to receive(:run_command).and_return(['safe', '', true])
    ...
    it 'handles instructions with quotes and special characters' do
      ...
      allow(Nitl::Interfaces::Shell).to receive(:run_command).and_return(['output', '', true])
    ...
    it 'does not raise on error by default' do
      allow(Nitl::Interfaces::Shell).to receive(:run_command).and_return(['error output', 'stderr', false])
    ...
    it 'uses raise_on_error: false when calling run_command' do
      ...
      expect(Nitl::Interfaces::Shell).to have_received(:run_command) do |cmd, options|
