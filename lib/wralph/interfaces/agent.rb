  ...
  module Interfaces
    module Agent
      ...
      def self.load_adapter
        config = Config.load
        source = config.agent_harness&.source || 'claude_code'
        ...
        when 'custom'
          load_custom_adapter(config.agent_harness.class_name)
        else
          raise "Unknown agent_harness source: #{source}"
        end
      end
